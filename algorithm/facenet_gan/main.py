...
if __name__ == '__main__':
    ...
    net = GAN(train=True).cuda(4)
    epoch = 0
    total_it = 0
    writer = SummaryWriter('/data/chenyangrui/gan/tsb/1')
    load = True
    if load:
        param = torch.load('/data/chenyangrui/gan/FaceGan_checkpoints')
        epoch = param['epoch']
    ...
    while(epoch <= 1000):
        ...
        torch.save({
            ...
            'D_loss': net.loss_D,
            'total_it': total_it
        }, '/data/chenyangrui/gan/FaceGan_checkpoints')
        if epoch % 10 == 0:
            torch.save(net.state_dict(), f"/data/chenyangrui/gan/FaceGan_{epoch}.pth")
