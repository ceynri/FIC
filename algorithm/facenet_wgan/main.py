...
if __name__ == '__main__':
    ...
    epoch = 0
    total_it = 0
    writer = SummaryWriter('/data/chenyangrui/wgan/crop/tsb')
    load = True
    if load:
        param = torch.load('/data/chenyangrui/wgan/crop/FaceWGangp_checkpoints')
        epoch = param['epoch']
    ...
    while(epoch <= 1000):
        ...
        torch.save({
            ...
            'G_loss': net.loss_G,
            'D_loss': net.loss_D
        }, '/data/chenyangrui/wgan/crop/FaceWGangp_checkpoints')
        if epoch % 10 == 0:
            torch.save(net.state_dict(), f"/data/chenyangrui/wgan/crop/FaceWGangp_{epoch}.pth")
